...
func (sp *ServerPeer) OnTx(p *peer.Peer, msg *wire.MsgTx) {
	...
	sp.server.blockManager.QueueTx(czzutil.NewTx(msg), sp)
}
...
func (sp *ServerPeer) OnFeeFilter(_ *peer.Peer, msg *wire.MsgFeeFilter) {
	...
	if msg.MinFee < 0 || msg.MinFee > czzutil.MaxSatoshi {
		log.Debugf("Peer %v sent an invalid feefilter '%v' -- "+
			"disconnecting", sp, czzutil.Amount(msg.MinFee))
		sp.Disconnect()
		return
	}
...
func (s *ChainService) RegisterMempoolCallback(onRecvTx func(tx *czzutil.Tx, block *btcjson.BlockDetails)) {
...
func (s *ChainService) NotifyMempoolReceived(addrs []czzutil.Address) {
	s.mempool.NotifyReceived(addrs)
}

// RequestMempoolFilter requests the mempool filter from a single peer.
func (s *ChainService) RequestMempoolFilter(addrs []czzutil.Address) {
