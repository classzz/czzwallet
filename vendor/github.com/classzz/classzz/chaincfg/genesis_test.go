...
func TestGetGenesisBlockHash(t *testing.T) {
	hash1 := genesisBlock.BlockHash()
	hash := hash1[:]
	fmt.Println(hex.EncodeToString(hash))

	for i := range hash {
		if (i+1)%8 == 0 {
			fmt.Println(fmt.Sprintf("0x%X,0x%X,0x%X,0x%X,0x%X,0x%X,0x%X,0x%X,",
				hash[i-7], hash[i-6], hash[i-5], hash[i-4], hash[i-3], hash[i-2], hash[i-1], hash[i]))
		}
	}
}
