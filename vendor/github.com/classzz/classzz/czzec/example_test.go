...
func Example_signMessage() {
	...
	privKey, pubKey := czzec.PrivKeyFromBytes(czzec.S256(), pkBytes)
...
func Example_verifySignature() {
	...
	pubKey, err := czzec.ParsePubKey(pubKeyBytes, czzec.S256())
	if err != nil {
		fmt.Println(err)
		return
	}
...
func Example_encryptMessage() {
	...
	message := "test message"
	ciphertext, err := czzec.Encrypt(pubKey, []byte(message))
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	privKey, _ := czzec.PrivKeyFromBytes(czzec.S256(), pkBytes)

	// Try decrypting and verify if it's the same message.
	plaintext, err := czzec.Decrypt(privKey, ciphertext)
	if err != nil {
		fmt.Println(err)
		return
	}
