...
// test data.
func genRandomSig() (*chainhash.Hash, *czzec.Signature, *czzec.PublicKey, error) {
	privKey, err := czzec.NewPrivateKey(czzec.S256())
	if err != nil {
...
func TestSigCacheAddExists(t *testing.T) {
	...
	sig1Copy, _ := czzec.ParseBERSignature(sig1.Serialize(), czzec.S256())
	key1Copy, _ := czzec.ParsePubKey(key1.SerializeCompressed(), czzec.S256())
...
func TestSigCacheAddEvictEntry(t *testing.T) {
	...
	for i := uint(0); i < sigCacheSize; i++ {
		...
		sigCopy, _ := czzec.ParseBERSignature(sig.Serialize(), czzec.S256())
		keyCopy, _ := czzec.ParsePubKey(key.SerializeCompressed(), czzec.S256())
	...
	sigNewCopy, _ := czzec.ParseBERSignature(sigNew.Serialize(), czzec.S256())
	keyNewCopy, _ := czzec.ParsePubKey(keyNew.SerializeCompressed(), czzec.S256())
