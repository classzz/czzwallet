...
// newAddressPubKey returns a new czzutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) czzutil.Address {
	addr, err := czzutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new czzutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) czzutil.Address {
	addr, err := czzutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new czzutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) czzutil.Address {
	addr, err := czzutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the czzutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the czzutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := czzutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := czzutil.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := czzutil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := czzutil.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := czzutil.NewAddressPubKey(hexToBytes("0411"+
	...
	tests := []struct {
		...
		{(*czzutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*czzutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*czzutil.AddressPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*czzutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*czzutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
