...
type addressToKey struct {
	key        *czzec.PrivateKey
	compressed bool
}

func mkGetKey(keys map[string]addressToKey) KeyDB {
	if keys == nil {
		return KeyClosure(func(addr czzutil.Address) (*czzec.PrivateKey,
			bool, error) {
...
func mkGetScript(scripts map[string][]byte) ScriptDB {
	if scripts == nil {
		return ScriptClosure(func(addr czzutil.Address) ([]byte, error) {
...
func TestGenAddress(t *testing.T) {
	...
	if tmp > 1 {
		...
		fmt.Println("AddressPubKeyHash:", address.String())
		_, err2 := czzutil.DecodeAddress(address.String(), &chaincfg.MainNetParams)
		if err2 != nil {
			fmt.Println(err2)
		}
	}
	if tmp > 2 {
		address, err1 := czzutil.NewAddressScriptHash(
			czzutil.Hash160(pk), &chaincfg.MainNetParams)

		if err1 != nil {
			t.Errorf("failed to make address for: %v", err1)
		}
		fmt.Println("AddressScriptHash:", address.String())
		_, err2 := czzutil.DecodeAddress(address.String(), &chaincfg.MainNetParams)
		if err2 != nil {
			fmt.Println(err2)
		}
	...
	if tmp > 0 {
		address, err1 := czzutil.NewLegacyAddressPubKeyHash(
			czzutil.Hash160(pk), &chaincfg.MainNetParams)

		if err1 != nil {
			t.Errorf("failed to make address for: %v", err1)
		}
		fmt.Println("LegacyAddressPubKeyHash:", address.String())
		_, err2 := czzutil.DecodeAddress(address.String(), &chaincfg.MainNetParams)
		if err2 != nil {
			fmt.Println(err2)
		}
	...
	if tmp > 3 {
		hash := czzutil.Hash160(pk)
		address, err1 := czzutil.NewLegacyAddressScriptHash(
		...
		str2 := address.String()
		// hash = append(hash,4)
		// str := base58.CheckEncode(hash[:], chaincfg.MainNetParams.LegacyScriptHashAddrID)
		fmt.Println("LegacyAddressScriptHash:", str2)
		_, err2 := czzutil.DecodeAddress(str2, &chaincfg.MainNetParams)
		if err2 != nil {
			fmt.Println(err2)
		}
	}
...
func TestSignTxOutput(t *testing.T) {
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			pk := (*czzec.PublicKey)(&key.PublicKey).
				SerializeUncompressed()
			address, err := czzutil.NewAddressPubKeyHash(
				czzutil.Hash160(pk), &chaincfg.TestNet3Params)
			if err != nil {
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			pk := (*czzec.PublicKey)(&key.PublicKey).
				SerializeUncompressed()
			address, err := czzutil.NewAddressPubKey(pk,
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			scriptAddr, err := czzutil.NewAddressScriptHash(
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			key1, err := czzec.NewPrivateKey(czzec.S256())
			if err != nil {
				t.Errorf("failed to make privKey for %s: %v",
					msg, err)
				break
			...
			pk1 := (*czzec.PublicKey)(&key1.PublicKey).
				SerializeCompressed()
			address1, err := czzutil.NewAddressPubKey(pk1,
				&chaincfg.TestNet3Params)
			if err != nil {
				t.Errorf("failed to make address for %s: %v",
					msg, err)
			...
			key2, err := czzec.NewPrivateKey(czzec.S256())
			if err != nil {
			...
			pk2 := (*czzec.PublicKey)(&key2.PublicKey).
				SerializeCompressed()
			address2, err := czzutil.NewAddressPubKey(pk2,
				&chaincfg.TestNet3Params)
			if err != nil {
			...
			pkScript, err := MultiSigScript(
				[]*czzutil.AddressPubKey{address1, address2},
				2)
			if err != nil {
				t.Errorf("failed to make pkscript "+
					"for %s: %v", msg, err)
...
var (
	...
	uncompressedAddrStr = "classzz:qrghed0t5spvk68qd9tt7vjnjq8q4pkflgdlgejcmk"
	compressedAddrStr   = "classzz:qqn5m8mlv9l8c7sur7e827gsgdjksfuascqhpjhttt"
)
...
func TestSignatureScript(t *testing.T) {
	...
	privKey, _ := czzec.PrivKeyFromBytes(czzec.S256(), privKeyD)
