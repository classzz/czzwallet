...
// a slice of them.
func loadBlocks(t *testing.T, dataFile string, network wire.BitcoinNet) ([]*czzutil.Block, error) {
...
func testFetchBlockIO(tc *testContext, tx database.Tx) bool {
	...
	for i, block := range tc.blocks {
		...
		// expected bytes.
		wantHeaderBytes := blockBytes[0:wire.BlockHeaderSize]
	...
	for i := 0; i < len(blockHeaderData); i++ {
		blockHash := allBlockHashes[i]
		wantHeaderBytes := allBlockBytes[i][0:wire.BlockHeaderSize]
