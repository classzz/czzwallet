...
func TestHeadersWire(t *testing.T) {
	...
	bh := NewBlockHeader(1, &hash, &merkleHash, &EmptyCIDRoot, bits, nonce)
	...
	tests := []struct {
		...
		// Protocol version 0 with no headers.
		{
			noHeaders,
			noHeaders,
			noHeadersEncoded,
		...
		// Protocol version 0 with one header.
		{
			oneHeader,
			oneHeader,
			oneHeaderEncoded,
...
func TestHeadersWireErrors(t *testing.T) {
	...
	// to force errors.
	bhTrans := NewBlockHeader(1, &hash, &merkleHash, &EmptyCIDRoot, bits, nonce)
