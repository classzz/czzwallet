...
// latest protocol version and decoding with 0.
func TestFilterAddCrossProtocol(t *testing.T) {
	...
	var buf bytes.Buffer
	err := msg.CzzEncode(&buf, ProtocolVersion, LatestEncoding)
	if err != nil {
		t.Errorf("encode of MsgFilterAdd failed %v err <%v>", msg, err)
	}
	...
	var readmsg MsgFilterAdd
	err = readmsg.CzzDecode(&buf, 0, LatestEncoding)
	if err == nil {
...
func TestFilterAddMaxDataSize(t *testing.T) {
	...
	err = msg.CzzDecode(readbuf, ProtocolVersion, LatestEncoding)
	if err == nil {
		t.Errorf("decode of MsgFilterAdd succeeded when it shouldn't "+
			"have %v", msg)
	}
...
func TestFilterAddWireErrors(t *testing.T) {
	...
	for i, test := range tests {
		...
		err = msg.CzzDecode(r, test.pver, test.enc)
