...
func TestVersionWire(t *testing.T) {
	// verRelayTxFalse and verRelayTxFalseEncoded is a version message
	// with the transaction relay disabled.
	...
	tests := []struct {
		...
		// Protocol version 0 with relay transactions field
		// true.
		{
			baseVersionBIP0037,
			baseVersionBIP0037,
...
func TestVersionWireErrors(t *testing.T) {
	...
	// Ensure calling MsgVersion.CzzDecode with a non *bytes.Buffer returns
	// error.
	fr := newFixedReader(0, []byte{})
	if err := baseVersion.CzzDecode(fr, pver, enc); err == nil {
		t.Errorf("Did not received error when calling " +
			"MsgVersion.CzzDecode with non *bytes.Buffer")
	}
	...
	tests := []struct {
		...
		// it's optional.
		{
			...
			0, BaseEncoding, 101, io.ErrShortWrite, nil,
		},
	...
	for i, test := range tests {
		...
		err := test.in.CzzEncode(w, test.pver, test.enc)
		if reflect.TypeOf(err) != reflect.TypeOf(test.writeErr) {
			t.Errorf("CzzEncode #%d wrong error got: %v, want: %v",
				i, err, test.writeErr)
			continue
		}
		...
		if _, ok := err.(*MessageError); !ok {
			if err != test.writeErr {
				t.Errorf("CzzEncode #%d wrong error got: %v, "+
		...
		err = msg.CzzDecode(buf, test.pver, test.enc)
		if reflect.TypeOf(err) != reflect.TypeOf(test.readErr) {
			t.Errorf("CzzDecode #%d wrong error got: %v, want: %v",
				i, err, test.readErr)
			continue
		}
		...
		if _, ok := err.(*MessageError); !ok {
			if err != test.readErr {
				t.Errorf("CzzDecode #%d wrong error got: %v, "+
...
// using protocol version and is used in the various tests.
