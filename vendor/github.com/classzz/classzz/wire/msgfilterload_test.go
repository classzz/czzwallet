...
// the latest protocol version and decoding with 0.
func TestFilterLoadCrossProtocol(t *testing.T) {
	...
	var buf bytes.Buffer
	err := msg.CzzEncode(&buf, ProtocolVersion, BaseEncoding)
	if err != nil {
	...
	err = readmsg.CzzDecode(&buf, 0, BaseEncoding)
	if err == nil {
...
func TestFilterLoadMaxFilterSize(t *testing.T) {
	...
	err = msg.CzzDecode(readbuf, ProtocolVersion, BaseEncoding)
	if err == nil {
