...
func BenchmarkDecodeGetHeaders(b *testing.B) {
	...
	var bb bytes.Buffer
	if err := m.CzzEncode(&bb, pver, LatestEncoding); err != nil {
		b.Fatalf("MsgGetHeaders.CzzEncode: unexpected error: %v", err)
	}
	buf := bb.Bytes()
	...
	for i := 0; i < b.N; i++ {
		r.Seek(0, 0)
		msg.CzzDecode(r, pver, LatestEncoding)
	}
}
...
func BenchmarkDecodeHeaders(b *testing.B) {
	...
	for i := 0; i < MaxBlockHeadersPerMsg; i++ {
		...
		m.AddBlockHeader(NewBlockHeader(1, hash, hash, &EmptyCIDRoot, 0, uint64(i)))
	}

	// Serialize it so the bytes are available to test the decode below.
	var bb bytes.Buffer
	if err := m.CzzEncode(&bb, pver, LatestEncoding); err != nil {
		b.Fatalf("MsgHeaders.CzzEncode: unexpected error: %v", err)
	}
	buf := bb.Bytes()
...
func BenchmarkDecodeGetBlocks(b *testing.B) {
	...
	var bb bytes.Buffer
	if err := m.CzzEncode(&bb, pver, LatestEncoding); err != nil {
		b.Fatalf("MsgGetBlocks.CzzEncode: unexpected error: %v", err)
	}
	buf := bb.Bytes()
...
func BenchmarkDecodeAddr(b *testing.B) {
	...
	// Serialize it so the bytes are available to test the decode below.
	var bb bytes.Buffer
	if err := ma.CzzEncode(&bb, pver, LatestEncoding); err != nil {
		b.Fatalf("MsgAddr.CzzEncode: unexpected error: %v", err)
	}
	buf := bb.Bytes()
...
func BenchmarkDecodeInv(b *testing.B) {
	...
	var bb bytes.Buffer
	if err := m.CzzEncode(&bb, pver, LatestEncoding); err != nil {
		b.Fatalf("MsgInv.CzzEncode: unexpected error: %v", err)
	}
	buf := bb.Bytes()
...
func BenchmarkDecodeNotFound(b *testing.B) {
	...
	var bb bytes.Buffer
	if err := m.CzzEncode(&bb, pver, LatestEncoding); err != nil {
		b.Fatalf("MsgNotFound.CzzEncode: unexpected error: %v", err)
	}
	buf := bb.Bytes()
...
func BenchmarkDecodeMerkleBlock(b *testing.B) {
	...
	m.Header = *NewBlockHeader(1, hash, hash, &EmptyCIDRoot, 0, uint64(10000))
	for i := 0; i < 105; i++ {
		hash, err := chainhash.NewHashFromStr(fmt.Sprintf("%x", i))
		if err != nil {
			b.Fatalf("NewHashFromStr: unexpected error: %v", err)
	...
	var bb bytes.Buffer
	if err := m.CzzEncode(&bb, pver, LatestEncoding); err != nil {
		b.Fatalf("MsgMerkleBlock.CzzEncode: unexpected error: %v", err)
	}
	buf := bb.Bytes()
