...
func makeSpendableOutForTx(tx *wire.MsgTx, txOutIndex uint32) spendableOut {
	return spendableOut{
		...
		amount: czzutil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
func makeTestGenerator(params *chaincfg.Params) (testGenerator, error) {
	privKey, _ := czzec.PrivKeyFromBytes(czzec.S256(), []byte{0x01})
...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := czzutil.Hash160(redeemScript)
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	...
	utilTxns := make([]*czzutil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, czzutil.NewTx(tx))
	}
...
func solveBlock(header *wire.BlockHeader) bool {
	...
	startNonce := uint64(1)
	stopNonce := uint64(math.MaxUint32)
...
func additionalCoinbase(amount czzutil.Amount) func(*wire.MsgBlock) {
...
func additionalSpendFee(fee czzutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
// tests.
func createSpendTx(spend *spendableOut, fee czzutil.Amount) *wire.MsgTx {
...
func createSpendTxForTx(tx *wire.MsgTx, fee czzutil.Amount) *wire.MsgTx {
...
func encodeNonCanonicalBlock(b *wire.MsgBlock) []byte {
	...
	b.Header.CzzEncode(&buf, 0, wire.BaseEncoding)
	buf.Write(nonCanonicalVarInt(uint32(len(b.Transactions))))
	for _, tx := range b.Transactions {
		tx.CzzEncode(&buf, 0, wire.BaseEncoding)
	}
	return buf.Bytes()
}
	...
	g.nextBlock("b41", outs[12], func(b *wire.MsgBlock) {
		...
		for i := 0; i < txnsNeeded; i++ {
			...
			sig, err := txscript.RawTxInECDSASignature(tx, 0,
				redeemScript, txscript.SigHashAll, g.privKey, int64(spend.amount.ToUnit(czzutil.AmountSatoshi)))
			if err != nil {
				panic(err)
			}
			tx.TxIn[0].SignatureScript = pushDataScript(sig,
...
func GenerateWithTxs(chainLength int, txsPerBlock int) (tests [][]TestInstance, err error) {
	...
	addAdditionalTxs := func(block *wire.MsgBlock) {
		// unsortedTxs is a slice of czzutil.Tx's that we will sort later using CTOR.
		var unsortedTxs []*czzutil.Tx

		// Let's first add any txs (excluding the coinbase) into unsortedTxs
		for _, tx := range block.Transactions[1:] {
			unsortedTxs = append(unsortedTxs, czzutil.NewTx(tx))
		}
...
func GenerateBlocksWithSchnorrSigs() (tests [][]TestInstance, err error) {
	...
	addAdditionalTxs := func(block *wire.MsgBlock) {
		...
		addr, err := czzutil.NewAddressPubKey(g.privKey.PubKey().SerializeCompressed(), &chaincfg.RegressionNetParams)
		if err != nil {
			panic(err)
		}
		...
		unsortedTxs = append(unsortedTxs, czzutil.NewTx(tx1))

		// Spend from tx1 using a Schnorr signature
		so1 := &spendableOut{
			amount: czzutil.Amount(tx1.TxOut[0].Value),
			prevOut: wire.OutPoint{
				Hash:  tx1.TxHash(),
				Index: 0,
			},
		...
		unsortedTxs = append(unsortedTxs, czzutil.NewTx(tx2))
		...
		addr2, err := czzutil.NewAddressScriptHash(redeemScript, &chaincfg.RegressionNetParams)
		if err != nil {
			panic(err)
		}
		...
		unsortedTxs = append(unsortedTxs, czzutil.NewTx(tx3))

		// Spend from tx3 using a push of a witness redeem script
		so2 := &spendableOut{
			amount: czzutil.Amount(tx3.TxOut[0].Value),
			prevOut: wire.OutPoint{
				Hash:  tx3.TxHash(),
				Index: 0,
			},
		...
		unsortedTxs = append(unsortedTxs, czzutil.NewTx(tx4))
