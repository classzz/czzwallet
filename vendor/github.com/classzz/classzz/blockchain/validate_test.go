...
func TestCheckConnectBlockTemplate(t *testing.T) {
	...
	err = chain.CheckConnectBlockTemplate(czzutil.NewBlock(&invalidPowBlock))
	if err != nil {
		t.Fatalf("CheckConnectBlockTemplate: Received unexpected error on "+
	...
	err = chain.CheckConnectBlockTemplate(czzutil.NewBlock(&invalidBlock))
	if err == nil {
		t.Fatal("CheckConnectBlockTemplate: Did not received expected error " +
...
func setValidMerkleRoot(b *wire.MsgBlock) {
	...
	utilTxns := make([]*czzutil.Tx, 0, len(b.Transactions))
...
func TestCheckBlockSanity(t *testing.T) {
	...
	block := czzutil.NewBlock(&Block100000)
