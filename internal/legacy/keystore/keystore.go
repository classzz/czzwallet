...
import (
	...
	"github.com/classzz/czzwallet/internal/legacy/rename"
)
...
func pubkeyFromPrivkey(privkey []byte, compress bool) (pubkey []byte) {
	_, pk := czzec.PrivKeyFromBytes(czzec.S256(), privkey)
...
func chainedPrivKey(privkey, pubkey, chaincode []byte) ([]byte, error) {
	...
	case czzec.PubKeyBytesLenUncompressed, czzec.PubKeyBytesLenCompressed:
		// Correct length
	default:
...
func chainedPubKey(pubkey, chaincode []byte) ([]byte, error) {
	var compressed bool
	switch n := len(pubkey); n {
	case czzec.PubKeyBytesLenUncompressed:
		compressed = false
	case czzec.PubKeyBytesLenCompressed:
		compressed = true
	default:
	...
	oldPk, err := czzec.ParsePubKey(pubkey, czzec.S256())
	if err != nil {
		return nil, err
	}
...
type Store struct {
	...
	passphrase       []byte
	secret           []byte
	chainIdxMap      map[int64]czzutil.Address
...
func New(dir string, desc string, passphrase []byte, net *chaincfg.Params,
	...
	s := &Store{
		...
		chainIdxMap:      make(map[int64]czzutil.Address),
...
func (s *Store) ReadFrom(r io.Reader) (n int64, err error) {
	...
	s.chainIdxMap = make(map[int64]czzutil.Address)
...
func (s *Store) NextChainedAddress(bs *BlockStamp) (czzutil.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
...
func (s *Store) ChangeAddress(bs *BlockStamp) (czzutil.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
...
func (s *Store) LastChainedAddress() czzutil.Address {
	s.mtx.RLock()
	defer s.mtx.RUnlock()
...
func (s *Store) Address(a czzutil.Address) (WalletAddress, error) {
	s.mtx.RLock()
	defer s.mtx.RUnlock()
...
func (s *Store) SetSyncStatus(a czzutil.Address, ss SyncStatus) error {
	s.mtx.Lock()
	defer s.mtx.Unlock()
...
func (s *Store) ImportPrivateKey(wif *czzutil.WIF, bs *BlockStamp) (czzutil.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
	...
	pkh := czzutil.Hash160(wif.SerializePubKey())
...
func (s *Store) ImportScript(script []byte, bs *BlockStamp) (czzutil.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
	...
	if _, ok := s.addrMap[addressKey(czzutil.Hash160(script))]; ok {
		return nil, ErrDuplicate
	}
...
type WalletAddress interface {
	// Address returns a czzutil.Address for the backing address.
...
func (s *Store) ActiveAddresses() map[czzutil.Address]WalletAddress {
	s.mtx.RLock()
	defer s.mtx.RUnlock()

	addrs := make(map[czzutil.Address]WalletAddress)
	for i := int64(rootKeyChainIdx); i <= s.highestUsed; i++ {
		a := s.chainIdxMap[i]
...
//
// A slice is returned with the czzutil.Address of each new address.
// The blockchain must be rescanned for these addresses.
func (s *Store) ExtendActiveAddresses(n int) ([]czzutil.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
...
type PubKeyAddress interface {
	...
	PrivKey() (*czzec.PrivateKey, error)
	// ExportPrivKey exports the WIF private key.
	ExportPrivKey() (*czzutil.WIF, error)
}
...
func newBchAddressWithoutPrivkey(s *Store, pubkey, iv []byte, bs *BlockStamp) (addr *bchAddress, err error) {
	...
	pk, err := czzec.ParsePubKey(pubkey, czzec.S256())
	if err != nil {
		return nil, err
	}

	address, err := czzutil.NewAddressPubKeyHash(czzutil.Hash160(pubkey), s.netParams())
	if err != nil {
		return nil, err
	}
...
func (a *bchAddress) ReadFrom(r io.Reader) (n int64, err error) {
	...
	addr, err := czzutil.NewAddressPubKeyHash(pubKeyHash[:], a.store.netParams())
	if err != nil {
		return n, err
	}
	a.address = addr
...
func (a *bchAddress) unlock(key []byte) (privKeyCT []byte, err error) {
	...
	x, y := czzec.S256().ScalarBaseMult(privkey)
...
func (a *bchAddress) Address() czzutil.Address {
	return a.address
}
...
// PubKeyAddress.
func (a *bchAddress) PubKey() *czzec.PublicKey {
	return a.pubKey
}
...
func (a *bchAddress) PrivKey() (*czzec.PrivateKey, error) {
...
func (a *bchAddress) ExportPrivKey() (*czzutil.WIF, error) {
	pk, err := a.PrivKey()
	if err != nil {
		return nil, err
	}
	...
	// elsewhere.
	wif, err := czzutil.NewWIF(pk, a.store.netParams(), a.Compressed())
	if err != nil {
		panic(err)
	}
	return wif, nil
...
func newScriptAddress(s *Store, script []byte, bs *BlockStamp) (addr *scriptAddress, err error) {
	...
	scriptHash := czzutil.Hash160(script)

	address, err := czzutil.NewAddressScriptHashFromHash(scriptHash, s.netParams())
	if err != nil {
		return nil, err
	}
...
func (sa *scriptAddress) ReadFrom(r io.Reader) (n int64, err error) {
	...
	address, err := czzutil.NewAddressScriptHashFromHash(scriptHash[:],
		sa.store.netParams())
	if err != nil {
		return n, err
	}
...
// address returns a czzutil.AddressScriptHash for a bchAddress.
func (sa *scriptAddress) Address() czzutil.Address {
	return sa.address
}
...
func (sa *scriptAddress) Addresses() []czzutil.Address {
	return sa.addresses
}
