...
// in a transaction.
type InputAmounts []czzutil.Amount
...
func (s *WithdrawalStatus) Fees() czzutil.Amount {
	return s.fees
}
...
func (o OutBailmentOutpoint) Amount() czzutil.Amount {
	return o.amount
}
...
type withdrawalTx struct {
	...
	// struct field instead of a method so that it can be replaced in tests.
	calculateFee func() czzutil.Amount
}
...
func newWithdrawalTx(setOptions func(tx *withdrawalTx)) *withdrawalTx {
	...
	tx.calculateFee = func() czzutil.Amount {
		return czzutil.Amount(1+tx.calculateSize()/1000) * feeIncrement
	}
	setOptions(tx)
	return tx
}
...
func (tx *withdrawalTx) inputTotal() (total czzutil.Amount) {
...
func (tx *withdrawalTx) outputTotal() (total czzutil.Amount) {
...
func (p *Pool) StartWithdrawal(ns walletdb.ReadWriteBucket, addrmgrNs walletdb.ReadBucket, roundID uint32, requests []OutputRequest,
	...
	txStore *wtxmgr.Store, txmgrNs walletdb.ReadBucket, chainHeight int32, dustThreshold czzutil.Amount) (
...
func (w *withdrawal) maybeDropRequests() {
	inputAmount := czzutil.Amount(0)
	for _, input := range w.eligibleInputs {
		inputAmount += input.Amount
	}
	outputAmount := czzutil.Amount(0)
...
func (wi *withdrawalInfo) match(requests []OutputRequest, startAddress WithdrawalAddress,
	lastSeriesID uint32, changeStart ChangeAddress, dustThreshold czzutil.Amount) bool {
...
func getWithdrawalStatus(p *Pool, ns, addrmgrNs walletdb.ReadBucket, roundID uint32, requests []OutputRequest,
	startAddress WithdrawalAddress, lastSeriesID uint32, changeStart ChangeAddress,
	dustThreshold czzutil.Amount) (*WithdrawalStatus, error) {
			...
			for i, pubKey := range pubKeys {
				...
				if privKey != nil {
					...
					sig, err = txscript.RawTxInECDSASignature(
						msgtx, inputIdx, redeemScript, txscript.SigHashAll, ecPrivKey, int64(input.Amount.ToUnit(czzutil.AmountSatoshi)))
					if err != nil {
...
func SignTx(msgtx *wire.MsgTx, inputAmounts InputAmounts, sigs TxSigs, mgr *waddrmgr.Manager, addrmgrNs walletdb.ReadBucket, store *wtxmgr.Store, txmgrNs walletdb.ReadBucket) error {
	...
	for i, pkScript := range pkScripts {
		if err = signMultiSigUTXO(mgr, addrmgrNs, msgtx, i, int64(inputAmounts[i].ToUnit(czzutil.AmountSatoshi)), pkScript, sigs[i]); err != nil {
			return err
		}
	}
	return nil
...
func getRedeemScript(mgr *waddrmgr.Manager, addrmgrNs walletdb.ReadBucket, addr *czzutil.AddressScriptHash) ([]byte, error) {
...
func signMultiSigUTXO(mgr *waddrmgr.Manager, addrmgrNs walletdb.ReadBucket, tx *wire.MsgTx,
	...
	redeemScript, err := getRedeemScript(mgr, addrmgrNs, addresses[0].(*czzutil.AddressScriptHash))
	if err != nil {
