...
func tstCreateSeriesCredits(t *testing.T, dbtx walletdb.ReadWriteTx, pool *Pool, seriesID uint32, amounts []int64) []Credit {
	...
	for i := range msgTx.TxOut {
		c := wtxmgr.Credit{
			...
			Amount:   czzutil.Amount(msgTx.TxOut[i].Value),
...
func TstNewOutputRequest(t *testing.T, transaction uint32, address string, amount czzutil.Amount,
	net *chaincfg.Params) OutputRequest {
	addr, err := czzutil.DecodeAddress(address, net)
	if err != nil {
...
func TstConstantFee(fee czzutil.Amount) func() czzutil.Amount {
	return func() czzutil.Amount { return fee }
}
...
func createAndFulfillWithdrawalRequests(t *testing.T, dbtx walletdb.ReadWriteTx, pool *Pool, roundID uint32) withdrawalInfo {
	...
	dustThreshold := czzutil.Amount(1e4)
