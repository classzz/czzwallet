...
func TestOutputSplittingNotEnoughInputs(t *testing.T) {
	...
	output1Amount := czzutil.Amount(2)
	output2Amount := czzutil.Amount(3)
...
func TestOutputSplittingOversizeTx(t *testing.T) {
	...
	requestAmount := czzutil.Amount(5)
	bigInput := int64(3)
	smallInput := int64(2)
	...
	if tx1.outputs[0].amount != czzutil.Amount(bigInput) {
	...
	if tx2.outputs[0].amount != czzutil.Amount(smallInput) {
...
func TestFulfillRequestsNoSatisfiableOutputs(t *testing.T) {
	...
	request := TstNewOutputRequest(
		t, 1, "3Qt1EaKRD9g9FeL2DGkLLswhK1AKmmXFSe", czzutil.Amount(3e6), pool.Manager().ChainParams())
	changeStart := TstNewChangeAddress(t, pool, seriesID, 0)
...
func TestFulfillRequestsNotEnoughCreditsForAllRequests(t *testing.T) {
	...
	out1 := TstNewOutputRequest(
		t, 1, "34eVkREKgvvGASZW7hkgE2uNc1yycntMK6", czzutil.Amount(3e6), net)
	out2 := TstNewOutputRequest(
		t, 2, "3PbExiaztsSYgh6zeMswC49hLUwhTQ86XG", czzutil.Amount(2e6), net)
	out3 := TstNewOutputRequest(
		t, 3, "3Qt1EaKRD9g9FeL2DGkLLswhK1AKmmXFSe", czzutil.Amount(5e6), net)
...
func TestRollbackLastOutputMultipleInputsRolledBack(t *testing.T) {
	...
	for i, amount := range []czzutil.Amount{4, 3, 2} {
...
func TestRollBackLastOutputInsufficientOutputs(t *testing.T) {
	...
	output := &WithdrawalOutput{request: TstNewOutputRequest(
		t, 1, "34eVkREKgvvGASZW7hkgE2uNc1yycntMK6", czzutil.Amount(3), &chaincfg.MainNetParams)}
...
func TestRollbackLastOutputWhenNewOutputAdded(t *testing.T) {
	...
	checkTxChangeAmount(t, firstTx, czzutil.Amount(4))
	...
	checkTxChangeAmount(t, secondTx, czzutil.Amount(3))
}
...
func TestWithdrawalTxAddChange(t *testing.T) {
	...
	tx.calculateFee = TstConstantFee(czzutil.Amount(fee))
...
func TestWithdrawalTxInputTotal(t *testing.T) {
	...
	if tx.inputTotal() != czzutil.Amount(5) {
		t.Fatalf("Wrong total output; got %v, want %v", tx.outputTotal(), czzutil.Amount(5))
	}
}
...
func TestWithdrawalTxOutputTotal(t *testing.T) {
	...
	if tx.outputTotal() != czzutil.Amount(4) {
		t.Fatalf("Wrong total output; got %v, want %v", tx.outputTotal(), czzutil.Amount(4))
	}
}
...
func TestSignMultiSigUTXOPkScriptNotP2SH(t *testing.T) {
	...
	mgr := pool.Manager()
	tx := createWithdrawalTx(t, dbtx, pool, []int64{4e6}, []int64{})
	addr, _ := czzutil.DecodeAddress("qr35ennsep3hxfe7lnz5ee7j5jgmkjswssk2puzvgv", mgr.ChainParams())
	pubKeyHashPkScript, _ := txscript.PayToAddrScript(addr.(*czzutil.AddressPubKeyHash))
	msgtx := tx.toMsgTx()
...
func TestSignMultiSigUTXORedeemScriptNotFound(t *testing.T) {
	...
	// script.
	addr, _ := czzutil.DecodeAddress("pzhxyrg99tjyd5gx2vgpzqfjh37ffwq38y3wm4qn5a", mgr.ChainParams())
	if _, err := mgr.Address(addrmgrNs, addr); err == nil {
		t.Fatalf("Address %s found in manager when it shouldn't", addr)
	}
	msgtx := tx.toMsgTx()

	pkScript, _ := txscript.PayToAddrScript(addr.(*czzutil.AddressScriptHash))
...
func TestSignMultiSigUTXONotEnoughSigs(t *testing.T) {
	...
	pkScript := tx.inputs[idx].PkScript
	TstRunWithManagerUnlocked(t, mgr, addrmgrNs, func() {
		err = signMultiSigUTXO(mgr, addrmgrNs, msgtx, idx, int64(txAmounts[idx].ToUnit(czzutil.AmountSatoshi)), pkScript, txInSigs)
	})
...
func TestSignMultiSigUTXOWrongRawSigs(t *testing.T) {
	...
	pkScript := tx.inputs[idx].PkScript
	TstRunWithManagerUnlocked(t, mgr, addrmgrNs, func() {
		err = signMultiSigUTXO(mgr, addrmgrNs, tx.toMsgTx(), idx, int64(txAmounts[idx].ToUnit(czzutil.AmountSatoshi)), pkScript, sigs)
	})
...
func TestTxSizeCalculation(t *testing.T) {
	...
	amounts := []czzutil.Amount{czzutil.Amount(1), czzutil.Amount(5)}
...
func TestStoreTransactionsWithChangeOutput(t *testing.T) {
	...
	inputTotal := czzutil.Amount(0)
	for _, debit := range txDetails.Debits {
		inputTotal += debit.Amount
	}
	if inputTotal != czzutil.Amount(5e6) {
		t.Fatalf("Unexpected input amount; got %v, want %v", inputTotal, czzutil.Amount(5e6))
	}
...
func createWithdrawalTxWithStoreCredits(t *testing.T, dbtx walletdb.ReadWriteTx, store *wtxmgr.Store, pool *Pool,
	...
	for i, amount := range outputAmounts {
		request := TstNewOutputRequest(
			t, uint32(i), "34eVkREKgvvGASZW7hkgE2uNc1yycntMK6", czzutil.Amount(amount), net)
...
func checkMsgTxOutputs(t *testing.T, msgtx *wire.MsgTx, requests []OutputRequest) {
	...
	for i, request := range requests {
		...
		gotAmount := czzutil.Amount(txOut.Value)
...
func signTxAndValidate(t *testing.T, mgr *waddrmgr.Manager, addrmgrNs walletdb.ReadBucket, tx *wire.MsgTx,
	...
	for i := range tx.TxIn {
		...
		TstRunWithManagerUnlocked(t, mgr, addrmgrNs, func() {
			if err := signMultiSigUTXO(mgr, addrmgrNs, tx, i, int64(amounts[i].ToUnit(czzutil.AmountSatoshi)),
...
func checkTxChangeAmount(t *testing.T, tx *withdrawalTx, amount czzutil.Amount) {
	if !tx.hasChange() {
...
func checkLastOutputWasSplit(t *testing.T, w *withdrawal, tx *withdrawalTx,
	origAmount, newAmount czzutil.Amount) {
