...
//
// NOTE: This is part of the chain.Interface interface.
func (c *BitcoindClient) NotifyReceived(addrs []czzutil.Address) error {
	_ = c.NotifyBlocks()
...
//	[]wire.OutPoint
//	[]*wire.OutPoint
//	map[wire.OutPoint]czzutil.Address
//	[]chainhash.Hash
//	[]*chainhash.Hash
func (c *BitcoindClient) LoadTxFilter(reset bool, filters ...interface{}) error {
	...
	for _, filter := range filters {
		...
		case []czzutil.Address, []wire.OutPoint, []*wire.OutPoint,
			map[wire.OutPoint]czzutil.Address, []chainhash.Hash,
			[]*chainhash.Hash:
...
func (c *BitcoindClient) Rescan(blockHash *chainhash.Hash,
	addresses []czzutil.Address, outPoints map[wire.OutPoint]czzutil.Address) error {
...
func (c *BitcoindClient) rescanHandler() {
	...
	for {
		...
		case update := <-c.rescanUpdate:
			...
			case map[wire.OutPoint]czzutil.Address:
				c.watchMtx.Lock()
				for op := range update {
					c.watchedOutPoints[op] = struct{}{}
				}
