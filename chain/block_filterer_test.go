...
func TestBlockFiltererOneInOneOut(t *testing.T) {
	...
	watchedOutPoints[firstTx.TxIn[0].PreviousOutPoint] = &czzutil.AddressPubKeyHash{}
	watchedOutPoints[lastTx.TxIn[0].PreviousOutPoint] = &czzutil.AddressPubKeyHash{}
...
func TestOutOfOrderDependentTransaction(t *testing.T) {
	addr, err := czzutil.DecodeAddress("qz2gcaj6dy2dg0e20tqh0k3v9a449h3a0s8t2kgggp", &chaincfg.SimNetParams)
	if err != nil {
		t.Fatal(err)
	}
	addr2, err := czzutil.DecodeAddress("qrpe3mafcwft5cqnchsyaeefw4000avtxg7jh5geku", &chaincfg.SimNetParams)
	if err != nil {
		t.Fatal(err)
	}
