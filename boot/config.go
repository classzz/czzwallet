...
import (
	...
	"github.com/classzz/czzwallet/internal/cfgutil"
...
const (
	defaultCAFilename       = "classzz.cert"
	defaultConfigFilename   = "czzwallet.conf"
	defaultLogLevel         = "info"
	defaultLogDirname       = "logs"
	defaultLogFilename      = "czzwallet.log"
...
var (
	czzdDefaultCAFile  = filepath.Join(czzutil.AppDataDir("classzz", false), "rpc.cert")
	defaultAppDataDir  = czzutil.AppDataDir("czzwallet", false)
...
type config struct {
	...
	// RPC client options
	RPCConnect       string                  `short:"c" long:"rpcconnect" description:"Hostname/IP and port of czzd RPC server to connect to (default localhost:8334, testnet: localhost:18334, simnet: localhost:18556)"`
	CAFile           *cfgutil.ExplicitString `long:"cafile" description:"File containing root certificates to authenticate a TLS connections with czzd"`
	DisableClientTLS bool                    `long:"noclienttls" description:"Disable TLS for the RPC client -- NOTE: This is only allowed if the RPC client is connecting to localhost"`
	CzzdUsername     string                  `long:"czzdusername" description:"Username for czzd authentication"`
	CzzdPassword     string                  `long:"czzdpassword" default-mask:"-" description:"Password for czzd authentication"`
	...
	Username               string                  `short:"u" long:"username" description:"Username for legacy RPC and czzd authentication (if czzdusername is unset)"`
	Password               string                  `short:"P" long:"password" default-mask:"-" description:"Password for legacy RPC and czzd authentication (if czzdpassword is unset)"`
...
//
// The above results in czzwallet functioning properly without any config
...
func loadConfig(optionalConfigPath *string) (*config, []string, error) {
	...
	if cfg.UseSPV {
		...
		//} else {。
		if !cfg.DisableClientTLS {
			// If CAFile is unset, choose either the copy or local czzd cert.
			if !cfg.CAFile.ExplicitlySet() {
				cfg.CAFile.Value = filepath.Join(cfg.AppDataDir.Value, defaultCAFilename)

				// If the CA copy does not exist, check if we're connecting to
				// a local czzd and switch to its RPC cert if it exists.
				...
				if !certExists {
					if _, ok := localhostListeners[RPCHost]; ok {
						czzdCertExists, err := cfgutil.FileExists(
							czzdDefaultCAFile)
						if err != nil {
							fmt.Fprintln(os.Stderr, err)
							return nil, nil, err
						...
						if czzdCertExists {
							cfg.CAFile.Value = czzdDefaultCAFile
						}
					}
				}
			}
	...
	// If the czzd username or password are unset, use the same auth as for
	// the client.  The two settings were previously shared for czzd and
	// client auth, so this avoids breaking backwards compatibility while
	// allowing users to use different auth settings for czzd and wallet.
	if cfg.CzzdUsername == "" {
		cfg.CzzdUsername = cfg.Username
	}
	if cfg.CzzdPassword == "" {
		cfg.CzzdPassword = cfg.Password
	}
