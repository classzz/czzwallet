...
func newFilterBlocksRequest(batch []wtxmgr.BlockMeta,
	...
	filterReq := &chain.FilterBlocksRequest{
		Blocks:           batch,
		ExternalAddrs:    make(map[waddrmgr.ScopedIndex]czzutil.Address),
		InternalAddrs:    make(map[waddrmgr.ScopedIndex]czzutil.Address),
...
// include a UTXO.
func (w *Wallet) CalculateBalance(confirms int32) (czzutil.Amount, error) {
...
type Balances struct {
	Total          czzutil.Amount
	Spendable      czzutil.Amount
	ImmatureReward czzutil.Amount
}
...
// address.
func (w *Wallet) PrivKeyForAddress(a czzutil.Address) (*czzec.PrivateKey, error) {
	var privKey *czzec.PrivateKey
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
		managedAddr, err := w.Manager.Address(addrmgrNs, a)
		if err != nil {
...
func (w *Wallet) HaveAddress(a czzutil.Address) (bool, error) {
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
...
func (w *Wallet) AccountOfAddress(a czzutil.Address) (uint32, error) {
	var account uint32
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
		var err error
...
func (w *Wallet) AddressInfo(a czzutil.Address) (waddrmgr.ManagedAddress, error) {
...
func listTransactions(tx walletdb.ReadTx, details *wtxmgr.TxDetails, addrMgr *waddrmgr.Manager,
	...
	if len(details.Debits) == len(details.MsgTx.TxIn) {
		...
		// is calculated.
		feeF64 = (outputTotal - debitTotal).ToCZZ()
	}
...
outputs:
	for i, output := range details.MsgTx.TxOut {
		...
		amountF64 := czzutil.Amount(output.Value).ToCZZ()
		...
		loopDetails:
			for i := range details {
				...
				for _, cred := range detail.Credits {
					...
					apkh, ok := addrs[0].(*czzutil.AddressPubKeyHash)
					if !ok {
						continue
					}
...
func (w *Wallet) Accounts(scope waddrmgr.KeyScope) (*AccountsResult, error) {
	...
	err = walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		...
		m := make(map[uint32]*czzutil.Amount)
...
func (w *Wallet) DumpPrivKeys() ([]string, error) {
	...
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		...
		// privkeys.
		return w.Manager.ForEachActiveAddress(addrmgrNs, func(addr czzutil.Address) error {
...
func (w *Wallet) DumpWIFPrivateKey(addr czzutil.Address) (string, error) {
...
func (w *Wallet) NewAddress(account uint32,
	scope waddrmgr.KeyScope) (czzutil.Address, error) {
...
func (w *Wallet) newAddress(addrmgrNs walletdb.ReadWriteBucket, account uint32,
	scope waddrmgr.KeyScope) (czzutil.Address, *waddrmgr.AccountProperties, error) {
...
// address.
func (w *Wallet) TotalReceivedForAddr(addr czzutil.Address, minConf int32) (czzutil.Amount, error) {
	var amount czzutil.Amount
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
...
func (w *Wallet) SignTransaction(tx *wire.MsgTx, inputValues []int64, hashType txscript.SigHashType,
	...
	err := walletdb.View(w.db, func(dbtx walletdb.ReadTx) error {
		...
		for i, txIn := range tx.TxIn {
			...
			getKey := txscript.KeyClosure(func(addr czzutil.Address) (*czzec.PrivateKey, bool, error) {
			...
			getScript := txscript.ScriptClosure(func(addr czzutil.Address) ([]byte, error) {
