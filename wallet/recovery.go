...
type RecoveryState struct {
	...
	// a rescan.
	watchedOutPoints map[wire.OutPoint]czzutil.Address
}
...
func NewRecoveryState(recoveryWindow uint32) *RecoveryState {
	...
	return &RecoveryState{
		recoveryWindow:   recoveryWindow,
		scopes:           scopes,
		watchedOutPoints: make(map[wire.OutPoint]czzutil.Address),
	}
}
...
func (rs *RecoveryState) WatchedOutPoints() map[wire.OutPoint]czzutil.Address {
...
type BranchRecoveryState struct {
	...
	addresses map[uint32]czzutil.Address
...
func NewBranchRecoveryState(recoveryWindow uint32) *BranchRecoveryState {
	return &BranchRecoveryState{
		recoveryWindow:  recoveryWindow,
		addresses:       make(map[uint32]czzutil.Address),
...
func (brs *BranchRecoveryState) GetAddr(index uint32) czzutil.Address {
...
func (brs *BranchRecoveryState) Addrs() map[uint32]czzutil.Address {
