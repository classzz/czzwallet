...
const DefaultRelayFeePerKb czzutil.Amount = 1e3
...
func GetDustThreshold(scriptSize int, relayFeePerKb czzutil.Amount) czzutil.Amount {
	...
	relayFee := czzutil.Amount(totalSize) * byteFee
	return 3 * relayFee
}
...
func IsDustAmount(amount czzutil.Amount, scriptSize int, relayFeePerKb czzutil.Amount) bool {
...
func IsDustOutput(output *wire.TxOut, relayFeePerKb czzutil.Amount) bool {
	...
	return IsDustAmount(czzutil.Amount(output.Value), len(output.PkScript),
		relayFeePerKb)
}
...
// output.
func CheckOutput(output *wire.TxOut, relayFeePerKb czzutil.Amount) error {
	if output.Value < 0 {
		return ErrAmountNegative
	}
	if output.Value > czzutil.MaxSatoshi {
...
func FeeForSerializeSize(relayFeePerKb czzutil.Amount, txSerializeSize int) czzutil.Amount {
	fee := relayFeePerKb * czzutil.Amount(txSerializeSize) / 1000

	if fee == 0 && relayFeePerKb > 0 {
		fee = relayFeePerKb
	}

	if fee < 0 || fee > czzutil.MaxSatoshi {
		fee = czzutil.MaxSatoshi
	}
