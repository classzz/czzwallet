...
func (w *Wallet) MakeMultiSigScript(addrs []czzutil.Address, nRequired int) ([]byte, error) {
	pubKeys := make([]*czzutil.AddressPubKey, len(addrs))
...
func (w *Wallet) ImportP2SHRedeemScript(script []byte) (*czzutil.AddressScriptHash, error) {
	var p2shAddr *czzutil.AddressScriptHash
	err := walletdb.Update(w.db, func(tx walletdb.ReadWriteTx) error {
		...
		if err != nil {
			...
			if waddrmgr.IsError(err, waddrmgr.ErrDuplicateAddress) {
				...
				p2shAddr, _ = czzutil.NewAddressScriptHash(script,
					w.chainParams)
				return nil
			}
			return err
		...
		p2shAddr = addrInfo.Address().(*czzutil.AddressScriptHash)
		return nil
	})
	return p2shAddr, err
}
