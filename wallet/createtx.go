...
func makeInputSource(eligible []wtxmgr.Credit) txauthor.InputSource {
	...
	currentTotal := czzutil.Amount(0)
	currentInputs := make([]*wire.TxIn, 0, len(eligible))
	currentScripts := make([][]byte, 0, len(eligible))
	currentInputValues := make([]czzutil.Amount, 0, len(eligible))

	return func(target czzutil.Amount) (czzutil.Amount, []*wire.TxIn,
...
func (s secretSource) GetKey(addr czzutil.Address) (*czzec.PrivateKey, bool, error) {
...
func (s secretSource) GetScript(addr czzutil.Address) ([]byte, error) {
	ma, err := s.Address(s.addrmgrNs, addr)
	if err != nil {
		return nil, err
	}
...
func (w *Wallet) txToOutputs(outputs []*wire.TxOut, account uint32,
	minconf int32, feeSatPerKb czzutil.Amount, dryRun bool) (
	...
	changeSource := func() ([]byte, error) {
		...
		var changeAddr czzutil.Address
		var err error
	...
	if tx.ChangeIndex >= 0 && account == waddrmgr.ImportedAddrAccount {
		changeAmount := czzutil.Amount(tx.Tx.TxOut[tx.ChangeIndex].Value)
...
func (w *Wallet) createUnsigned(outputs []*wire.TxOut, account uint32,
	minconf int32, feeSatPerKb czzutil.Amount) (tx *txauthor.AuthoredTx, err error) {
...
func validateMsgTx(tx *wire.MsgTx, prevScripts [][]byte, inputValues []czzutil.Amount) error {
