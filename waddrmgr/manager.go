...
func (m *Manager) Address(ns walletdb.ReadBucket,
	address czzutil.Address) (ManagedAddress, error) {
...
func (m *Manager) MarkUsed(ns walletdb.ReadWriteBucket, address czzutil.Address) error {
	m.mtx.RLock()
	defer m.mtx.RUnlock()
...
func (m *Manager) AddrAccount(ns walletdb.ReadBucket,
	address czzutil.Address) (*ScopedKeyManager, uint32, error) {
...
func (m *Manager) ForEachActiveAddress(ns walletdb.ReadBucket, fn func(addr czzutil.Address) error) error {
	m.mtx.RLock()
	defer m.mtx.RUnlock()
...
func createManagerKeyScope(ns walletdb.ReadWriteBucket,
	...
	// The address manager needs the public extended key for the account.
	importAcctKeyPub, err := acctKeyPriv.Neuter()
	if err != nil {
		str := "failed to convert private key for account 0"
		return managerError(ErrKeyChain, str, err)
	}
	...
	return putAccountInfo(
		ns, &scope, ImportedAddrAccount, importAcctPubEnc, importAcctPrivEnc, 0, 0,
