...
// Address returns the czzutil.Address which represents the managed address.
// This will be a pay-to-pubkey-hash address.
//
// This is part of the ManagedAddress interface implementation.
func (a *managedAddress) Address() czzutil.Address {
	return a.address
}
...
func (a *managedAddress) PubKey() *czzec.PublicKey {
	return a.pubKey
}
...
//
// This is part of the ManagedPubKeyAddress interface implementation.
func (a *managedAddress) PrivKey() (*czzec.PrivateKey, error) {
	...
	privKey, _ := czzec.PrivKeyFromBytes(czzec.S256(), privKeyCopy)
...
//
// This is part of the ManagedPubKeyAddress interface implementation.
func (a *managedAddress) ExportPrivKey() (*czzutil.WIF, error) {
	pk, err := a.PrivKey()
	if err != nil {
		return nil, err
	}

	return czzutil.NewWIF(pk, a.manager.rootManager.chainParams, a.compressed)
}
...
func newManagedAddressWithoutPrivKey(m *ScopedKeyManager,
	derivationPath DerivationPath, pubKey *czzec.PublicKey, compressed bool,
	...
	if compressed {
		...
		pubKeyHash = czzutil.Hash160(pubKey.SerializeUncompressed())
	}

	var address czzutil.Address
	var err error
	...
	case RawPubKey:
		if compressed {
			address, err = czzutil.NewAddressPubKey(pubKey.SerializeCompressed(), m.rootManager.chainParams)
		} else {
			address, err = czzutil.NewAddressPubKey(pubKey.SerializeUncompressed(), m.rootManager.chainParams)
		}
	}
...
func newManagedAddress(s *ScopedKeyManager, derivationPath DerivationPath,
	privKey *czzec.PrivateKey, compressed bool,
	addrType AddressType) (*managedAddress, error) {
	...
	ecPubKey := (*czzec.PublicKey)(&privKey.PublicKey)
...
type scriptAddress struct {
	manager         *ScopedKeyManager
	account         uint32
	address         *czzutil.AddressScriptHash
...
//
// This is part of the ManagedAddress interface implementation.
func (a *scriptAddress) Address() czzutil.Address {
	return a.address
}
...
func newScriptAddress(m *ScopedKeyManager, account uint32, scriptHash,
	...
	address, err := czzutil.NewAddressScriptHashFromHash(
