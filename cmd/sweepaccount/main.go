...
var (
	walletDataDirectory = czzutil.AppDataDir("bchwallet", false)
	newlineBytes        = []byte{'\n'}
)
...
func makeInputSource(outputs []btcjson.ListUnspentResult) txauthor.InputSource {
	var (
		totalInputValue czzutil.Amount
		inputs          = make([]*wire.TxIn, 0, len(outputs))
		inputValues     = make([]czzutil.Amount, 0, len(outputs))
		sourceErr       error
	)
	...
	return func(czzutil.Amount) (czzutil.Amount, []*wire.TxIn, []czzutil.Amount, [][]byte, error) {
...
func sweep() error {
	...
	for _, previousOutputs := range sourceOutputs {
		...
		outputAmount := czzutil.Amount(tx.Tx.TxOut[0].Value)
...
func saneOutputValue(amount czzutil.Amount) bool {
	return amount >= 0 && amount <= czzutil.MaxSatoshi
}
