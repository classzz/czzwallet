...
import (
	...
	"github.com/classzz/czzwallet/pymtproto/payments"
...
func (c *PaymentProtocolClient) DownloadBip0070PaymentRequest(uri string) (*PaymentRequest, error) {
	...
	for _, out := range paymentDetails.Outputs {
		...
		output := Output{
			Address: addrs[0],
			Amount:  czzutil.Amount(int64(*out.Amount)),
		}
...
func (c *PaymentProtocolClient) PostPayment(payment *Payment) (memo string, err error) {
	...
	for _, tx := range payment.Transactions {
		...
		if err := tx.CzzEncode(&buf, 0, wire.BaseEncoding); err != nil {
			return "", err
		}
	...
	refundAmount := uint64(payment.RefundOutput.Amount.ToUnit(czzutil.AmountSatoshi))
