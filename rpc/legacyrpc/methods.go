...
func makeMultiSigScript(w *wallet.Wallet, keys []string, nRequired int) ([]byte, error) {
	keysesPrecious := make([]*czzutil.AddressPubKey, len(keys))
	...
	for i, a := range keys {
		...
		default:
			...
			pubKeyAddr, err := czzutil.NewAddressPubKey(
...
func addMultiSigAddress(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	secp256k1Addrs := make([]czzutil.Address, len(cmd.Keys))
...
func createMultiSig(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	address, err := czzutil.NewAddressScriptHash(script, w.ChainParams())
	if err != nil {
...
func decodeAddress(s string, params *chaincfg.Params) (czzutil.Address, error) {
	addr, err := czzutil.DecodeAddress(s, params)
	if err != nil {
...
func getUnconfirmedBalance(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	return (bals.Total - bals.Spendable).ToCZZ(), nil
}
...
func importPrivKey(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	wif, err := czzutil.DecodeWIF(cmd.PrivKey)
	if err != nil {
		return nil, &btcjson.RPCError{
			Code:    btcjson.ErrRPCInvalidAddressOrKey,
...
func getReceivedByAccount(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	return results[acctIndex].TotalReceived.ToCZZ(), nil
}
...
func getTransaction(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	var (
		debitTotal  czzutil.Amount
		creditTotal czzutil.Amount // Excludes change
		fee         czzutil.Amount
		feeF64      float64
	)
	...
	if len(details.Debits) == len(details.MsgTx.TxIn) {
		var outputTotal czzutil.Amount
		for _, output := range details.MsgTx.TxOut {
			outputTotal += czzutil.Amount(output.Value)
		}
	...
	if len(details.Debits) == 0 {
		...
		ret.Details[0] = btcjson.GetTransactionDetailsResult{
			...
			Category: "send",
			Amount:   (-debitTotal).ToCZZ(), // negative since it is a send
			Fee:      &feeF64,
		}
		ret.Fee = feeF64
	}
	...
	for _, cred := range details.Credits {
		...
		ret.Details = append(ret.Details, btcjson.GetTransactionDetailsResult{
			...
			Address:  address,
			Category: credCat,
			Amount:   cred.Amount.ToCZZ(),
			Vout:     cred.Index,
		})
	}

	ret.Amount = creditTotal.ToCZZ()
	return ret, nil
}
...
func listAccounts(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	for _, result := range results {
		accountBalances[result.AccountName] = result.AccountBalance.ToCZZ()
	}
...
func listReceivedByAccount(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	for _, result := range results {
		jsonResults = append(jsonResults, btcjson.ListReceivedByAccountResult{
			...
			Amount:        result.TotalReceived.ToCZZ(),
...
// and amounts.
func makeOutputs(pairs map[string]czzutil.Amount, chainParams *chaincfg.Params) ([]*wire.TxOut, error) {
...
func sendPairs(w *wallet.Wallet, amounts map[string]czzutil.Amount,
	account uint32, minconf int32, feeSatPerKb czzutil.Amount) (string, error) {
...
func sendFrom(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	...
	amt, err := czzutil.NewAmount(cmd.Amount)
	if err != nil {
		return nil, err
	}
	pairs := map[string]czzutil.Amount{
		cmd.ToAddress: amt,
	}
...
func sendMany(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	pairs := make(map[string]czzutil.Amount, len(cmd.Amounts))
	for k, v := range cmd.Amounts {
		amt, err := czzutil.NewAmount(v)
		if err != nil {
			return nil, err
		}
		pairs[k] = amt
...
func signMessage(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	sigbytes, err := czzec.SignCompact(czzec.S256(), privKey,
		messageHash, true)
	if err != nil {
		return nil, err
	}
...
func signRawTransaction(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	...
	var keys map[string]*czzutil.WIF
	if cmd.PrivKeys != nil {
		keys = make(map[string]*czzutil.WIF)

		for _, key := range *cmd.PrivKeys {
			wif, err := czzutil.DecodeWIF(key)
			if err != nil {
				return nil, DeserializationError{err}
			}
...
func verifyMessage(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	pk, wasCompressed, err := czzec.RecoverCompact(czzec.S256(), sig,
		expectedMessageHash)
	if err != nil {
		return nil, err
	}
	...
	case *czzutil.AddressPubKeyHash: // ok
		return bytes.Equal(czzutil.Hash160(serializedPubKey), checkAddr.Hash160()[:]), nil
	case *czzutil.AddressPubKey: // ok
